<h1 align="center" id="start">
    Kotlin. Руководство для начинающих
</h1>
...
<ul>
<li>Клонируй проект: git clone https://github.com/IvankovVal/tenzor_test_task.git</li>
<li>Открой в IntelliJ IDEA</li>
<li>Запускай примеры из всех глав руководства</li>
<li>Решай задания, которые найдёшь в файлах TasksForModul_*.kt, по ссылкам</li>
</ul>

<p>
Мы ожидаем, что на изучение этого руководства ты потратишь не больше месяца.
На этот период в конце каждого рабочего дня ждём письмо с отчётом о проделанной за день работе.
Тема письма в формате "стажёр Имя Фамилия - 01.01.2026". В письме расскажи с чем ты справился за день 
и с чем у тебя трудности чтобы куратор обучения оперативно пришёл к тебе на помощь. 
В конце каждой недели тебя ждёт встреча с куратором обучения для обсуждения пройденных глав руководства. 
Результатом твоей работы за этот месяц будет твёрдо усвоенная теория об основах
программирования на Kotlin, несколько небольших выполненных заданий для отработки навыков программирования
и большое консольное приложение выполненное в объектно-ориентированном стиле.
Подробности о всех заданиях ты найдёшь в каждой главе руководства.
</p>



📞 Связь с куратором обучения
<div>
...
читаемость и гарантирует точность, особенно в сложных API. 
Например, <code>val height: Double = 1.80</code> четко фиксирует тип Double. Иначе можно полагаться на инференцию, 
как в случае <code>val city = "Таганрог"</code>, когда компилятор самостоятельно определяет тип из контекста. Явные объявления бывают необходимы, когда тип должен строго коррелировать с логикой приложения или отличаться от типа присвоенного значения.
</p>
<p>
...
<a href="src\TasksForModul_3.kt">Задания для закрепления материала</a>
...
```kotlin
val age = 18 //создание переменной
if (age >= 18) { //в круглых скобках логическое выражение, которое выступает условием
    println("Доступ разрешен") //команда, которая будет выполнена
} else {
...
val a = 10
val b = 20
val max = if (a > b) a else b //использование if в качестве тернарного оператора
...
if (score >= 90) {
    ...
    println("Хорошо") //будет выведено на консоль
} else {
...
<p>
Оператор <b>when</b> - это элегантное решение, заменяющее цепочки <b>if-else</b>, 
когда они становятся громоздкими. 
...
when (day) {
    ...
    2 -> println("Вторник")
    3 -> println("Среда") //будет выведено на консоль
    else -> println("Другой день") //альтернативный случай, будет напечатан, если ни одно условие не подойдёт
}
```

<p>Группировка нескольких условий. 
Через запятую можно указать несколько значений для одной ветки выполнения.</p>
...
<p>Использование диапазонов (in)
Проверка вхождения числа в определенный интервал.</p>

```kotlin
val score = 75
when (score) {
    in 90..100 -> println("Оценка: А")
    in 70..89 -> println("Оценка: B")
    in 50..69 -> println("Оценка: C")
    else -> println("Нужно пересдать")
}
```
<p>when как выражение. 
Результат выполнения ветки можно сразу присвоить переменной. 
В этом случае ветка else обязательна.</p>
...
<p>Проверка типа данных (is).
when позволяет удобно проверять типы объектов.</p>

```kotlin
val x: Any = "Hello"
when (x) {
    is Int -> println("Это целое число")
    is String -> println("Это строка длиной ${x.length}")
    else -> println("Неизвестный тип")
}
```

<p>when без аргументов.
Если не передавать переменную в when, он работает как цепочка if-else if.</p>

```kotlin
val hour = 15
when {
    hour < 12 -> println("Доброе утро")
    hour < 18 -> println("Добрый день")
    else -> println("Добрый вечер")
}
```
<p>
Существуют логические операторы, которые широко используются в Kotlin для комбинирования
и инвертирования условий, что позволяет строить сложные проверки в if и when.
</p>
<ul>
<li><code>&&</code> - оператор логического «И»</li>
<li><code>||</code> - оператор логического «ИЛИ»</li>
<li><code>!</code> оператор логического отрицания</li>
</ul>
<p>
...
<br><a href="src\TasksForModul_4.kt">Задания для закрепления материала</a>
